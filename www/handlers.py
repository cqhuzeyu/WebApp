...
def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uid, name=name.strip(), email=email, passed=hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(), image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email.encode('utf-8')).hexdigest())
...
def authenticate(*, email, passwd):
    ...
    if user.passwd != sha1.hexdigest():
        raise APIValueError('passwd', 'Invalide password.')
